...
public class AppTest {
    ...
    @Test
    void testEditarContacto() {
        agenda.añadirContacto(contacto1);
        agenda.editarContacto(contacto1, "Juan Editado", "Alias1Editado", "Calle Editada", "111111Editado", "juaneditado@mail.com");
        Contacto editado = agenda.buscarContacto(contacto1);
        assertEquals("Juan Editado", editado.getNombres(), "El nombre del contacto debería haberse editado correctamente");
        assertEquals("Alias1Editado", editado.getAlias(), "El alias del contacto debería haberse editado correctamente");
    }
    ...
    @Test
    void testEditarGrupo() {
        agenda.añadirGrupo(grupo1);
        agenda.editarGrupo(grupo1, "Amigos Editado", Categoria.AMIGOS, new Contacto[]{contacto1, contacto2});
        Grupo editado = agenda.buscarGrupo(grupo1);
        assertEquals("Amigos Editado", editado.getNombre(), "El nombre del grupo debería haberse editado correctamente");
        assertEquals(Categoria.AMIGOS, editado.getCategoria(), "La categoría del grupo debería haberse editado correctamente");
        assertEquals(2, editado.getListaContactosPertenecientes().length, "El grupo debería tener 2 contactos después de la edición");
    }
    ...
    @Test
    void testEditarReunion() {
        agenda.añadirReunion(reunion1);
        agenda.editarReunion(reunion1, "Reunion Editada", LocalDate.now().plusDays(1), Time.valueOf("16:00:00"), new Contacto[]{contacto1, contacto2});
        Reunion editada = agenda.buscarReunion(reunion1);
        assertEquals("Reunion Editada", editada.getDescripcion(), "La descripción de la reunión debería haberse editado correctamente");
        assertEquals(Time.valueOf("16:00:00"), editada.getHora(), "La hora de la reunión debería haberse editado correctamente");
        assertEquals(2, editada.getListaContactosAsistentes().length, "La reunión debería tener 2 asistentes después de la edición");
    }
